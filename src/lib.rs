...
#[wasm_bindgen]
pub fn solve_string(puzzle_string: &str, multi: bool) -> String {
  let puzzle = build_puzzle(puzzle_string);
  if multi {
    let solved = solve_sudoku_multi(puzzle);
    return solved;
  } else {
    let solved = solve_sudoku(&mut puzzle);
    return solved;
  }
}

pub fn solve_string_multithread(puzzle_string: &str) -> String {
...
fn solve_sudoku_multi(grid: Vec<Square>) -> String {
  let threadpool = ThreadPool::new(4);
  let guesses: &mut Vec<Vec<Square>> = &mut vec![];
  guesses.push(grid);
  let (tx, rx) = mpsc::channel();
  while guesses.len() > 0 {
    let current_grid = guesses.pop().unwrap();
    let tx = tx.clone();
    threadpool.execute(move || loop {
      let current_grid = &mut current_grid.clone();
      solve_iteration(current_grid);
      tx.send(current_grid);
    });
  }
  let iterated = rx.recv().unwrap();
  let solve_status = check_solved_state(iterated);
  match solve_status {
    SudokuState::Solved => {
      return destruct_puzzle(&iterated);
    }
    SudokuState::NeedsGuess => {
      let applied_guesses = &mut apply_guess(&iterated);
      guesses.append(applied_guesses);
    }
    SudokuState::InProgress => {
      guesses.push(iterated.to_vec());
    }
    SudokuState::Invalid => {}
  }
}
...
mod tests {
  ...
  fn test_really_hard_sudoku() {
    ...
    let solved = solve_string(test_string);
    let valid_solved =
      "148395672735628914692741358951472863427836195386159427863517249274963581519284736";
    assert_eq!(solved, valid_solved);
  }
